...
r_vec_list<-list(c(2.0, 1.2,1.3, 1.4 ,1.5,1.6),
                 ...
                 c(2.0, 2,2.1,2.2,2.3,2.4),
                 c(2.0, 1.2,1.4,1.6,1.8,2),
                 c(2.0, 1.6,1.8,2,2.2,2.4),
                 c(2.0,1.2,1.6,2,2.4,2.8))
...
#epistasis_matrix[epistasis_matrix < 0.001] <-0
    ...
    for (repl in 1:replicas){
      ...
      for (w in gen){
        ...
        if (n==1){
          ...
          # Esta hecho asi para que si no hay de un genotipo que aparezca un cero
          ...
          for (ph in 1:length(phenotypes)){
            ...
            if (sample_data[ph,2]>0){
              ...
              # si no se ha pickeado el genoypo en concreto entonces no hay de esos genotipos
              x[]=0
              final_x_total<-cbind(final_x_total,x)}
